...
func New(cfg config.Config, logger *log.Logger, mediaService *media.Service) *App {
	...
	e.POST("/api/photos/views", handler.TrackViews)
...
func serveMedia(baseDir string, logger *log.Logger) echo.HandlerFunc {
	return func(c echo.Context) error {
		p := c.Param("*")
		if p == "" {
		...
		logger.Debug("serving media file", "path", filePath)
